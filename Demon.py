        ...
        for unparsed_phone in str_list:
            ...
            if not client.is_user_authorized():
                try:
                    ...
                    MadeByLegend_dev.append(Nero_op)
                    continue
...
def Scraper():
    ...
    link1 = (config['Legend_dev/Smitmore2']['FromGroup']).strip()
    links = link1.split(',')
    phone = (config['Legend_dev/Smitmore2']['PhoneNumber']).strip()
...
def DailyFilter():
    ...
    print(Style.BRIGHT + Fore.RESET + 'Welcome To @Legend_dev Program\n')
    ...
    config = configparser.ConfigParser()
    config.read("config.ini")
    links = (config['Legend_dev/Smitmore2']['FromGroup']).strip()
    link = links.split(',')
    phone = (config['Legend_dev/Smitmore2']['PhoneNumber']).strip()
...
def DeleteALreadyMembers():
    ...
    config = configparser.ConfigParser()
    config.read("config.ini")
    link = (config['Legend_dev/Smitmore2']['ToGroup']).strip()
    phone = (config['Legend_dev/Smitmore2']['PhoneNumber']).strip()
...
def banner():
    os.system('clear')
    print(f"""
{re} _____ _____ _     _____ ____ ____      _    __  __        _    ____  ____  _____ ____
{re}|_   _| ____| |   | ____/ ___|  _ \    / \  |  \/  |      / \  |  _ \|  _ \| ____|  _  |
{cy}  | | |  _| | |   |  _|| |  _| |_) |  / _ \ | |\/| |     / _ \ | | | | | | |  _| | |_) |
{cy}  | | | |___| |___| |__| |_| |  _ <  / ___ \| |  | |    / ___ \| |_| | |_| | |___|  _ <
{re}  |_| |_____|_____|_____\____|_| \_\/_/   \_\_|  |_|   /_/   \_\____/|____/|_____|_| \_\
{cy}                                           
{cy}          Version : 1.01                   
{cy}                                 BY @DEMON_NETWORK          
{cy}                                               
 {re}If any any one try to Sell you this script
   {gr}Contact me on telegram @smitmore2
        """)
...
def DeleteContact():
    ...
    Legend_dev_ne_script_banaya_hai = int(input('From Account No : ')) - 1
    telegram_script_banyane_ke_liye_Legend_dev_ko_dm_kro = int(input('Upto Account No : '))
    indexx = 0
    global Legend_devpro
    Legend_devpro = 0
    for Legend_devonyt in phlist[Legend_dev_ne_script_banaya_hai:telegram_script_banyane_ke_liye_Legend_dev_ko_dm_kro]:
        indexx += 1
        print(f'Index : {indexx}')
        phone = utils.parse_phone(Legend_devonyt)
        print(f"Login {phone}")
        client = TelegramClient(f"sessions/{phone}", API_ID, HashID)
        client.start(phone)
...
def BulkAdder():
    ...
    config = configparser.ConfigParser()
    config.read("config.ini")
    grouplink = config['Legend_dev/Smitmore2']['ToGroup']
    groupid = config['Legend_dev/Smitmore2']['GroupID']
    stopnum = config['Legend_dev/Smitmore2']['EnterStop']
    stacno = config['Legend_dev/Smitmore2']['StartingAccount']
    endacno = config['Legend_dev/Smitmore2']['EndAccount']
    ...
    upto = int(endacno)
    indexx = 0
    global Legend_devdev_is_main_developer
    Legend_devdev_is_main_developer = 0
    for pythondeveloper in phlist[start_from:upto]:
        ...
        try:
            ...
            # This Script Is Made My T.Me/Legend_dev.
            elif prchk == 'N':
                client(JoinChannelRequest(prlink))
                time.sleep(5)
                channel = client.get_input_entity(PeerChannel(id))
        channelinfo = client(GetFullChannelRequest(channel=channel))
        Legend_devdev_is_main_developer = int(channelinfo.full_chat.participants_count)
        print(f'Members: {Legend_devdev_is_main_developer}')
        if Legend_devdev_is_main_developer >= stop:
        ...
        while batchcount < countcon:
            semen = [delta for delta in user_to_add[:Legend_devdevismain]]
            # print(f'Left {len(lit)}')
            try:
                time.sleep(Legend_dev_dev)
                client(functions.channels.InviteToChannelRequest(
                    channel=id,
                    users=semen
                ))
                # print(f'Added : {added}')
                batchcount += Legend_devdevismain
                for i in range(0, 10):
                    try:
                        del user_to_add[i]
...
def SingleAdder():
    ...
    if prchk == 'Y':
        ...
        prlink = grouplink
    stop = int(stopnum)
    Legend_dev_ne_script_banaya_hai = int(stacno) - 1
    telegram_script_banyane_ke_liye_Legend_dev_ko_dm_kro = int(endacno)
    indexx = 0
    global Legend_devpro
    Legend_devpro = 0
    for deltaxd in phlist[Legend_dev_ne_script_banaya_hai:telegram_script_banyane_ke_liye_Legend_dev_ko_dm_kro]:
        indexx += 1
        print(f'Index : {indexx}')
        ...
        try:
            ...
            if prchk == 'Y':
                ...
                channel = client.get_input_entity(PeerChannel(id))
        channelinfo = client(GetFullChannelRequest(channel=channel))
        Legend_devpro = int(channelinfo.full_chat.participants_count)
        print(f'Members: {Legend_devpro}')
        if Legend_devpro >= stop:
            print(f'The Goal Of {stop} Has Been Completed')
            input()
            quit()
        contacts = client(GetContactsRequest(hash=0))
...
def Adder():
    ...
    Legend_dev_devinput = int(input())


    with open('phone.csv', 'r') as read_obj:
        csv_reader = reader(read_obj)
        list_of_rows = list(csv_reader)
        row_number = Legend_dev_devinput
        col_number = 1
    ...
    config = configparser.ConfigParser()
    config.read("config.ini")
    to_group = config['Legend_dev/Smitmore2']['ToGroup']
...
def main_menu():
    ...
    if a==1:
        ...
        print(re+'Developer : @Legend_dev / @Smithmore2')
        if b==1:
            login()
        elif b==2:
            BanFilter()
